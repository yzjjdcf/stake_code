...
# ================= 7. 启动后同步 dialogs =================
@app.on_connect()
async def sync_dialogs_on_connect(client):
    """连接后立即同步 dialogs，清除无效的 Peer ID"""
    try:
        logger.info("🔄 正在同步 dialogs/peers，清除无效的频道 ID...")
        await client.get_dialogs()
        logger.info("✅ dialogs 同步完成，无效的频道 ID 已清除")
    except Exception as e:
        logger.warning(f"⚠️ 同步 dialogs 时出错（不影响运行）: {e}")
