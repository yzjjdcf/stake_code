...
class WinnaClaimRecord(models.Model):
    """记录 Winna 网站的抢码结果（完全独立于 Stake 业务）"""
    user_id = models.CharField(max_length=100, null=True, blank=True, verbose_name="用户标识", help_text="用户唯一标识符，用于区分不同使用者（一个用户可以有多个账号）", db_index=True)
    username = models.CharField(max_length=100, null=True, blank=True, verbose_name="账号", help_text="Winna 账号用户名字符串，用于 WebSocket 领取记录")
    code = models.CharField(max_length=100, verbose_name="代码", db_index=True)
    ...
    class Meta:
        verbose_name = "领取明细（Winna）"
        verbose_name_plural = "领取明细（Winna）"
        ordering = ['-created_at']
        indexes = [
        ...
        # 使用独立的数据库表名，避免与 Stake 业务冲突
        db_table = 'winna_claimrecord'
